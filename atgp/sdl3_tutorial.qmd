...
std::cout << SDL_GetNumJoysticks() << std::endl;

SDL_Joystick* joystick = SDL_OpenJoystick(SDL_GetJoysticks(nullptr)[0]);
...
if (joystick == nullptr) {
    ...
    std::cout << "Num Track balls: " << SDL_GetNumJoystickBalls(joystick) << std::endl;
...
for (i = 0; i < SDL_GetNumJoysticks(); ++i) {
    if (SDL_IsGamepad(i)) {
        ...
        std::cout << "Index '" << i << "' is a compatible controller, named '"
                  << SDL_GetGamepadNameForID(SDL_GetGamepads(nullptr)[i]) << "'" << std::endl;
        gamepad = SDL_OpenGamepad(SDL_GetGamepads(nullptr)[i]);
...
For instance, if you have a PlayStation 4 controller plugged into your computer, this code has similar output to the joystick example.
...
int main(int argc, char* argv[]) {
    ...
    SDL_FRect drawingRect{0, 0, 640, 480};
...
int main(int argc, char* argv[]) {
    ...
    currentImage = LoadTexture("galaxy2.bmp", renderTarget);
