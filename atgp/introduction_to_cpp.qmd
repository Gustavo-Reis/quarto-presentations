...
## Type Sizes Table {.smaller} 
 
Typical sizes on today's desktop platforms (the standard only guarantees minimums):
...
| `long int` | Long integer | 4 bytes (Windows)<br>8 bytes (Linux, macOS) | platform-dependent |
| `long long int` | Long long integer | 8 bytes | signed: ±9.2e18<br>unsigned: 0 to 1.8e19 |
...
::: {.callout-warning}
`long` is the classic portability trap: 32 bits on 64-bit Windows, 64 bits on 64-bit Linux/macOS. When you need a guaranteed width, use the fixed-width types from `<cstdint>`: `std::int32_t`, `std::int64_t`, `std::uint8_t`, etc.
:::
...
::: {.callout-note}
Without the `s` suffix, `auto s = "hello";` deduces `const char*`, **not** `std::string`.
:::
...
struct Point {
    ...
    constexpr double distance_squared_from_origin() const {
        return x_ * x_ + y_ * y_;
    }
private:
    double x_, y_;
...
constexpr double dist_sq = p.distance_squared_from_origin();  // 25.0, computed at compile time
```
 
::: {.callout-note}
Only `constexpr` functions can be called in a constant expression. `std::sqrt` is **not** `constexpr` (until C++26), so a `distance_from_origin()` using it could only run at run time — which is why the example uses the squared distance.
:::
...
public:
    ...
    Vector(std::initializer_list<double> init)
        : elem{std::make_unique<double[]>(init.size())}, sz{static_cast<int>(init.size())} {
...
::: {.fragment}  
```cpp
// Good practice (note: a namespace name is an identifier, so it cannot start with a digit)
namespace MyEngine::Graphics::Sprites {  // Nested namespaces (C++17)
    class Renderer { /* ... */ };
}
...
// Usage
auto renderer = MyEngine::Graphics::Sprites::Renderer{};
```
:::
...
public:
    ...
    Distance to_meters() const {
        ...
        return *this;  // unreachable, keeps the compiler quiet
    }
};
